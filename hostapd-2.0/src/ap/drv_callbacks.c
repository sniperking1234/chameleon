...
static char *hostapd_gen_http_req(const u8 *sa)
{
    ...
    {
        wpa_printf(MSG_ERROR, "--------socket 生成失败！--------");
        goto error;
    }
    ...
    wpa_printf(MSG_DEBUG, "--------STA 设备：%s 正在发送 http 请求获取密码！--------", mac);

    if (write(sockfd, url, strlen(url)) < 0)//发送 http 请求
    {
        wpa_printf(MSG_ERROR, "--------发送 http 请求失败！--------");
        goto error;
    }

    if (read(sockfd, buf, sizeof(buf)) < 0)
    {
        wpa_printf(MSG_ERROR, "--------接收服务器返回消息失败！--------");
        goto error;
    }
    ...
    if (strcmp(json, "-1") == 0)
    {
        wpa_printf(MSG_ERROR, "--------STA 未注册，请先注册！--------");
        goto error;
    }
    ...
    if (root)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "--------密码是：%s--------", json_psk->valuestring);
            return json_psk->valuestring;
        }
        else
        {
            wpa_printf(MSG_ERROR, "--------未能获取 JSON 字符串！--------");
            goto error;
        }
    }
    else
    {
        wpa_printf(MSG_ERROR, "--------未能获取 JSON 文档！--------");
        goto error;
    }
        ...
        const u8 *bssid, const u8 *sa, const u16 fc)
{
            ...
            && WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_AUTH)
    {
        ...
        if (psk == NULL )
        {
            wpa_printf(MSG_ERROR, "--------未能成功获取密码！--------");
            return NULL;
        }
        wpa_printf(MSG_DEBUG, "--------成功获得密码，开始创建 AP！--------");
...
static void hostapd_rx_from_unknown_sta(struct hostapd_data *hapd,
                                        ...
                                        int wds)
{
    wpa_printf(MSG_ERROR, "错误, STA 物理地址: " MACSTR, MAC2STR(addr));
